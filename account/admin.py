...
from account.models import Registration, Role
...
class MyUserAdmin(UserAdmin):
  ...
  fieldsets = [
    ...
    (_('Personal info'), {'fields': ('first_name','last_name','dob','country','gender','contact')}),
    (
      _('Permissions'),
      {
        'fields':(
  ...
  add_fieldsets = (
    (None, {'classes': ('wide',), 'fields':('email','contact','country','dob','gender','password1','password2')}),
  )
    ...
    for user in obj.user_set.all():
      ...
      links.append(
        """<a href="{}" target="_blank">{}</a>""".format(
          url, user.get_full_name() or user.first_name,
        )
      )
